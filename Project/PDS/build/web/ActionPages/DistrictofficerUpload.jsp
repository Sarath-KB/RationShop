<%-- 
    Document   : OfficerUploadAction.jsp
    Created on : 29 Jan, 2020, 10:19:46 PM
    Author     : USER
--%>
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            ...
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Assets\\DPhoto\\";//path for storing image
            Random r=new Random();//to generate random number
             int r_num=r.nextInt(1111)+999;//values between this are generated
             
            fn="Image_"+r_num+ext;//generating random number for images so this name and format of imagewil be stored inside the server
           ...
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            //setting path to store image
            File proj_path=new File(config.getServletContext().getRealPath("/"));
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Assets\\DProof\\";//path for storing image
            Random r=new Random();//to generate random number
             int r_num=r.nextInt(1111)+999;//values between this are generated
             
            fn="proof_"+r_num+ext;//generating random number for images so this name and format of imagewil be stored inside the server
            //creating a file object
            savedFile=new File(file_path+fn);//fn contains name of file
             try
            {
...
if(value[5].equals(value[6])){
   String str1="insert into tbl_districtofficer(districtofficer_name,districtofficer_contact,districtofficer_email,district_id,districtofficer_photo,districtofficer_proof,districtofficer_username,districtofficer_password)values('"+value[0]+"','"+value[1]+"','"+value[2]+"','"+value[3]+"','"+fn+"','"+fm+"','"+value[4]+"','"+value[5]+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='../Admin/Districtofficer.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
}else{
