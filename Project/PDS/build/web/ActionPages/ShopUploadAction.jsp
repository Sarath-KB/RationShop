...
if(isMultipart)
{
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            ...
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Shopkeeper\\Images\\";//path for storing image
           ...
           //to upload proof of doctor
            ...
            if(field_name.equals("txtlicenseproof"))//write the name of image here it is img_product
           {
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            //setting path to store image
            File proj_path=new File(config.getServletContext().getRealPath("/"));
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Shopkeeper\\Proof\\";//path for storing image
            Random r=new Random();//to generate random number
             int r_num=r.nextInt(1111)+999;//values between this are generated
   ...
   String str1="insert into tbl_shop(shop_name,shop_address,shop_contact,shop_email,shop_shopno,shop_licenseproof,location_id,shop_photo,shop_username,shop_password,supplyofficer_id)values('"+value[0]+"','"+value[1]+"','"+value[2]+"','"+value[3]+"','"+value[4]+"','"+proof+"','"+value[5]+"','"+ fn +"','"+value[6]+"','"+value[7]+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='../Supplyofficer/Shop.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
     }    
