    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            ...
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Admin\\Photo\\";//path for storing image
...
if(value[5].equals(value[6])){
   String str1="insert into tbl_talukofficer(talukofficer_name,talukofficer_contact,talukofficer_email,place_id,talukofficer_photo,talukofficer_proof,talukofficer_username,talukofficer_password)values('"+value[0]+"','"+value[1]+"','"+value[2]+"','"+value[3]+"','"+fn+"','"+fm+"','"+value[4] +"','"+value[5]+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='../DistrictOfficer/TalukOfficer.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
}else{
