<%-- 
    Document   : CardrequestUpload
    Created on : 02-Jun-2021, 23:52:34
    Author     : hp -pc
--%>
...
if(isMultipart)
{
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            //setting path to store image
            File proj_path=new File(config.getServletContext().getRealPath("/"));
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Assets\\RPhoto\\";//path for storing image
            Random r=new Random();//to generate random number
             int r_num=r.nextInt(1111)+999;//values between this are generated
   ...
   String str1="insert into tbl_cardrequest(cardrequest_name,cardrequest_address,card_income,cardrequest_nos,cardrequest_photo,cardrequest_proof,user_id)values('"+value[0]+"','"+value[1]+"','"+value[2]+"','"+value[3]+"','"+fn+"','"+proof+"','"+session.getAttribute("sid")+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='CardRequest.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
     }    
